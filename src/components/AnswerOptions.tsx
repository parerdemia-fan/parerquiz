...
export const AnswerOptions: React.FC<AnswerOptionsProps> = ({
  ...
  return (
    <div className="space-y-1 lg:space-y-4">
      {/* 選択肢グリッド */}
      <div className="grid grid-cols-2 gap-0.5 lg:gap-4 p-0 lg:p-2">
        {options.map((talent, index) => (
          <button
            ...
            disabled={isAnswered}
            className={`
              relative rounded-lg lg:rounded-xl font-medium font-rounded transition-all duration-300 shadow-lg hover:shadow-xl
          ...
          >
            {gameMode === 'name' ? (
              ...
              <div>
                {!isAnswered ? (
                  <div className="w-full aspect-[4/3] lg:aspect-square flex flex-col items-center justify-center bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 rounded-md lg:rounded-lg relative overflow-hidden border-2 border-indigo-200/50 shadow-inner">
                    {/* 装飾的な背景パターン */}
                    ...
                    {/* メインコンテンツ - 縦方向中央揃えに調整 */}
                    <div className="relative z-10 flex flex-col items-center justify-center h-full text-center p-1 lg:p-4">
                      {/* カナ表示 - 名前の上に配置 */}
                      <div className={`${getKanaFontSize(talent.kana)} font-medium text-indigo-500/80 mb-1 leading-tight`}>
                        {talent.kana}
                      </div>
                      {/* 名前表示 - 縦方向完全中央、カナ削除 */}
                      <div className={`${getOptionNameFontSize(talent.name)} font-black bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 bg-clip-text text-transparent leading-tight drop-shadow-sm whitespace-nowrap px-1 lg:px-2`}>
                        {talent.name}
                      </div>
                    </div>
                  ...
                  <div>
                    <img
                      ...
                      alt={talent.name}
                      className="w-full aspect-[4/3] lg:aspect-square object-cover rounded-md lg:rounded-lg no-drag"
                      onError={(e) => {
                    ...
                    />
                    <div className="absolute bottom-1 left-1 right-1 bg-black/60 text-white px-2 py-1 rounded-b-md lg:rounded-b-lg">
